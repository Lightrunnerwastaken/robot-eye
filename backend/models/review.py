...
class Review(Base):
    ...
    content_id: Mapped[int] = mapped_column(ForeignKey("content_units.id", ondelete="CASCADE"), index=True)
    ease: Mapped[int] = mapped_column(Integer, nullable=False, default=1)
    next_due: Mapped[datetime] = mapped_column(DateTime, nullable=False, default=datetime.utcnow, index=True)
